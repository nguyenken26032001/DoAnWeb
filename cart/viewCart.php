            ...
            <div id="head" class="panel panel-primary">
              ...
              foreach ($cart as $item) : ?>
                            <form action="cart.php" method="POST">
                                <tr>
                                    ...
                                    <td style="text-align: center;"><input type="number" name="quantity" id="quantity"
                                            value="<?php print_r($item['num']) ?>" min="1"
                                            max="<?php echo $item['sltk'] ?>"> </td>
                                    <td style="text-align: center;">
                                        <select style="width:70px;" id="size" name="size">
                                            <?php
                        foreach ($mangSize as $value) {
                          if ($item['size'] == $value) {
                        ...
                        ?>
                                    ...
                                    <td style="text-align: center;">
                                        <?php echo   number_format($item['gia'] * $item['num']) ?> VNĐ</td>
                                    <input type="hidden" name="id" id="id" value="<?php echo $item['id'] ?>">
                                    <td><button class="btn btn-warning" name="action" value="update"> UPDATE</button>
                                    </td>
                            </form>
                            <td><button type="button" class="btn btn-danger"
                                    onclick="deleteData(<?php echo $item['id'] ?>)"> DELETE</button> </td>
                            </tr>
                        </tbody>
                        <?php endforeach;
          ?>
